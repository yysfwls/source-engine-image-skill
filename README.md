...
- 有参考图时执行风格转换，并积极保持原图内容关系。
- 没有参考图时根据文字描述创建新的游戏场景。
- 默认不添加 HUD、准星、菜单、调试文字、水印、怪物或血腥元素。
- 默认输出为完整的旧游戏截图质感，而不是只给主体套一层低模滤镜。

## 文件说明

- `SKILL.md`：供 Codex 读取的核心执行规则、提示词骨架与质量检查标准。
- `agents/openai.yaml`：Skill 在 Codex 界面中的名称、简介和默认调用提示。
- `README.md`：供安装者和使用者阅读，不参与模型的核心执行流程。

## 为什么暂时没有 references

当前 Skill 只有一种核心任务，全部必要规则可以在较短的 `SKILL.md` 中一次读完。此时增加 `references/` 会把同一套逻辑拆散，并不会减少上下文成本。

当后续增加多套独立预设时，再建立 `references/` 更合适，例如：
...
届时应在 `SKILL.md` 中按任务类型选择性读取对应文件，避免每次加载所有预设。
